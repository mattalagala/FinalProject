...
function getCategoryList () {
  ...
  INSERT INTO category (category_id, category_name, category_description, category_team_name, category_languages, category_picture)
  VALUES (?,?,?,?,?,?)
  RETURNING *;
  `

  function createProject (newProjectName, newProjectDescription, newTeamName, languagesUsed) {
    
    return new Promise (function (resolve, reject) {
      conn.raw(addProjectQuery, [uuid(), newProjectName, newProjectDescription, newTeamName, languagesUsed, 'https://picsum.photos/620/620'])
      .then((result) => {
        console.log(uuid())
        return (result.rows)
      })
